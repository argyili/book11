...
public final class Scanner implements Iterator<String>, Closeable {
    ...
    public String next() {
        ...
        while (true) {
            ...
            if (token != null) {
                ...
                System.out.println(token); // Liang 
                return token;
            }
            if (needInput)
                readInput();
    ...
    public byte nextByte(int radix) {
        ...
        try {
            ...
            byte val = Byte.parseByte(s, radix);
            System.out.println(val); // Liang
    ...
    public short nextShort(int radix) {
        ...
        try {
            ...
            if (matcher.group(SIMPLE_GROUP_INDEX) == null)
                s = processIntegerToken(s);
            short val = Short.parseShort(s, radix);
            System.out.println(val); // Liang
    ...
    public long nextLong(int radix) {
        ...
        try {
            ...
            if (matcher.group(SIMPLE_GROUP_INDEX) == null)
                s = processIntegerToken(s);
            long val = Long.parseLong(s, radix); // Liang
    ...
    public float nextFloat() {
        ...
        clearCaches();
        try {
          float val = Float.parseFloat(processFloatToken(next(floatPattern())));
          System.out.println(val); // Liang
            return val;
        } catch (NumberFormatException nfe) {
            position = matcher.start(); // don't skip bad token
    ...
    public double nextDouble() {
        ...
        if ((typeCache != null) && (typeCache instanceof Double)) {
            ...
            useTypeCache();
            System.out.println(val); // Liang change, 9/12/07
            return val;
        }
        setRadix(10);
        clearCaches();
        ...
        try {
          double val = Double.parseDouble(processFloatToken(next(floatPattern())));
          System.out.println(val); // Liang change, 9/12/07
          return val;
        } catch (NumberFormatException nfe) {
            position = matcher.start(); // don't skip bad token
